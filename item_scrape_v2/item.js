...
async function run(name) {
  ...
  const names = await firstDiv[0].$$eval('.item-title', (elems) => elems.map((e) => e.innerText));
    names.shift();

const percentages = await firstDiv[0].$$eval('.percent-chance', (elems) => elems.map((e) => e.innerText));
percentages.shift();
...
async function printPercentage() {
    ...
    for (var i in names_object) {
        ...
        excel_object.push({itemName: name, monsterName: monsterName, percentage: parseFloat(percent ?? 0)})
    }
...
async function runAndPrint() {
    if (!fs.existsSync(OUTPUT_DIRECTORY)) {
      ...
      console.log("Starting monster", monsterName);
